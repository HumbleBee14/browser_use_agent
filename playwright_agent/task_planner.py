...
- The agent has these actions: goto, click, type, scroll, screenshot, extract, wait, done, fail.
...
async def plan(instruction: str) -> tuple[TaskSpec, list[SampleInput]]:
    ...
    # Parse the JSON response — handle markdown wrapping, extra text, etc.
    ...
    # Strategy 2: Find first { to last } (in case of surrounding text)
    ...
    # Build TaskSpec
    spec_data = data["task_spec"]
    # Fill in defaults for fields the planner might skip
